...
| LeetCode 959  | Regions Cut By Slashes                                       | Medium     | First we should divide a square into 4 parts by its two diagonal (see the comment in the code). Then we can do a mapping to map the `4*n*n` elements into an array by `4n*i + 4*j + t` . Then we can do UnionFind. When we iterate the grids, we only union the inner partitions and the previous partitions (that means only union the upper partition and left partition with their neighbors). For counting distinct groups, we can use a counter variable `count` inside the UnionFind class and decrease it by one when the results of `find()` in `union()` are not equal. **Note that** we can also use DFS to solve this question. That is divide each of the original grid into a 3*3 square. Then the question is like `Number of Islands`. See [C++ with picture, DFS on upscaled grid](https://leetcode.com/problems/regions-cut-by-slashes/discuss/205674/C%2B%2B-with-picture-DFS-on-upscaled-grid). |
...
| **LeetCode 430** | Flatten a Multilevel Doubly Linked List | Medium     | **Good question**. The key is to treat this linked list as a binary tree. `child` represents the left subtree, `next` represents the right subtree. And this question becomes a **preorder** traversal. The `helper()` function returns the tail of the flattened linked list. |
...
| LeetCode 845 | Longest Mountain in Array                                    | Medium     | Extend the window until we get a valid mountain, then record the maximum length. One exception input case is all full of the same number, like `[2, 2, 2, 2, 2, 2]`. For this case, we must ensure that we have been ascending then descending. **Another approach** is to iterate the orignial array  from the head to the tail and from the tail to the head by two-pass. Record two prefix length arrays in this way. |
...
| LeetCode 1366 | Rank Teams by Votes        | Medium     | We have at most `votes[0].length()` rounds and it is less than 26. So we can use a 26 * 26 array to record the votes each letter gets in each round. Then we can use this array to sort the original character set. |
...
| LeetCode 873            | Length of Longest Fibonacci Subsequence          | Medium     | **Good Question**. This question's state definition is a kind of different. In order to infer the next item of the current Fibonacci Seq, we need to record the last two elements. Thus, the state definition of this question is: `dp[i][j] => maximum length of the sequence ends with i and j`. The state transition equation is `dp[i][j] = dp[k][i] + 1       k, i in [0, j-1] and num[k] + num[i] = num[j]`. The state initialization is `dp[i][j] = 2, for every i < j`. |
| Microsoft 2019 China Q2 | Minimum Shooting                                 | Hard       | Similar to `317 Burst Balloons`. State definition: `state definition: dp[i][j] => the minimal cost of bursting all bottles from i to j`. State transition: `dp[i][j] = min(dp[i+1][j-1](if nums[i] == nums[j]), dp[i][k]+dp[k+1][j])`. State initialization: `dp[i][j] = 1, i = j`. While updating states, we should start from the ones with shorter length. |
| Microsoft 2019 China Q1 | Pass Ball                                        | Medium     | We solve this question by modifying the original DFS solution to the memoization search version. We always return the answer instead of `void dfs()` in memoization search. |
| LeetCode 294            | Flip Game II                                     | Medium     | **Game Theory**. Use memorization to speed up. First write a function to generate possible candidates of next state. Then recursively call the helper function to get whether starting from the given pattern can win. The key condition is that if one candidate can not win, the current pattern can win. |
...
| LeetCode  647 | Palindromic Substrings                                       | Medium     | Use a `boolean[][]` array to record whether the substring from i to j is a palindrome. Then count the palindrome substrings from length of 1 to length of n. In total, it takes O(n^2) time. |
| LeetCode 361  | Bomb Enemy                                                   | Medium     | Like the game `bomb man`. We can use a `rowCnt` and an array `colCnt[j]` to record the enemies at the current coordinate we can kill. When we meet a "Wall" grid or it's at the start row or column (`i == 0 or j == 0`), we should initialize `rowCnt` and `colCnt[j]`. |
...
| LeetCode 1227 | Airplane Seat Assignment Probability | Medium     | Probability formula derivation.                              |
