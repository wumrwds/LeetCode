class Solution {
    /*
    greedy
    In each step, we choose a largest subsequence in source to cover the target
    ...
    */
    public int shortestWay(String source, String target) {
        ...
        while (start < n) {
            for (end = Math.min(start + m, n-1); end >= start; end--) {
                if (isSubsequence(source, target, start, end)) {
                    count++;
                    break;
                }
            }
    ...
    private boolean isSubsequence(String source, String target, int start, int end) {
        int i = 0, j = start;
        while (j <= end && i < source.length()) {
